...
Functional equivalence was demonstrated for the safety-critical core. Two defects remain open in the non-safety diagnostic-logging path — filesystem timing starvation on the SD card interface and truncation of the SPI payload — and neither affects the safety-critical domain. Formal methods and hardware-in-the-loop test rigs were outside the project scope.
