...
\begin{table}[!t]
  ...
  \begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}}lllr@{}}
    ...
    SPI (32-bit)                & L2 & Fixture/USB  & Open$^*$ \\
    SD card                     & L2 & Firmware      & Open$^\dagger$ \\
    \bottomrule
    \multicolumn{4}{@{}p{\linewidth}@{}}{\scriptsize $^*$Truncated to 16 bits by the STM32 receive configuration; sizing the reception to the full 32-bit frame remains open work.} \\
    \multicolumn{4}{@{}p{\linewidth}@{}}{\scriptsize $^\dagger$Blocked by filesystem mounting delays in the write loop; mounting once at initialisation and decoupling logging from the receive interrupt remains open work.}
  \end{tabular*}
\end{table}
...
\begin{table}[!t]
  ...
  \begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}}llp{2.3cm}lll@{}}
    ...
    B4 & Firmware& FatFS timing starvation blocking SPI Rx           & L2 & Fixture/YAML  & Open \\
    B5 & Interface& SPI payload truncation (16-bit vs 32-bit buffer)  & L2 & Fixture/USB   & Open \\
    \bottomrule
  \end{tabular*}
\end{table}
...
After re-routing the STM32 Serial Wire Debug (SWD) pins and debugging the firmware state machine~\cite{savchuk2025}, diagnostic data routing was validated via \USB{} serial streaming, confirming isolation integrity. \SPI{} packet reception and SD logging remain subject to the open defects of Section~\ref{sec:limitations}.

\subsection{Open Integration Issues}
\label{sec:limitations}

The discovery of the following integration issues during Layer~2 testing validates the necessity of the scenario-based approach; these faults would not have been detected by Layer~1 simulation alone. Both remain open in the non-safety diagnostic path:
\begin{itemize}[nosep]
  \item \textbf{SD Card Storage (FatFS Timing Starvation):} The FatFS implementation on the STM32 causes firmware timing starvation during block write attempts to the MicroSD card. Root-cause analysis showed that the firmware unconditionally mounts and unmounts the filesystem volume on every write loop cycle, which introduces a \SIrange{50}{200}{\milli\second} blocking overhead. This file system overhead, combined with blocking delays in the main execution thread, starves the SPI receive interrupt re-arming window, resulting in data overruns. Streaming over the \USB{} CDC interface bypasses the SDIO path and avoids the blocking; the identified fix is to mount the volume once at initialisation, remove the loop-level mounting calls, and decouple logging from the receive interrupt.
  \item \textbf{\SPI{} Payload Truncation and Alignment:} The VHDL SPI master correctly transmits a 32-bit diagnostic frame (consisting of a 2-bit event-type prefix, 6-bit button state, and 24-bit error code). However, the STM32 firmware is configured to receive only 16 bits per transaction (\texttt{HAL\_SPI\_Receive\_IT} is called with a buffer size of 2). This mismatch truncates the payload and leaves the remaining 16 bits unread. The leftover clock pulses trigger a receiver buffer overrun (\texttt{OVR} flag in the SPI status register) and cause persistent frame misalignment on subsequent transfers. The identified fix is to size the reception to the full 32-bit frame; because the F401 SPI peripheral supports only 8- or 16-bit data frames, this requires a four-byte buffer at 8-bit data width rather than a single 32-bit transfer.
